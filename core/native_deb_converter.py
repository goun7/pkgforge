...
class NativeDebConverter(QObject):
    ...
    def convert(self, deb_path: Path, output_dir: Path) -> None:
        ...
        try:
            ...
            # Unknown upstream license → honest LicenseRef + stub file.
            from core.maps import arch_license, write_license_stub

            lic = arch_license(
                getattr(self._meta, "license", "") or "", self._meta.name
            )
            if lic.startswith("LicenseRef-"):
                write_license_stub(src_dir, self._meta.name)

            pkgbuild_content = self._generate_pkgbuild(self._meta, resolved_deps, lic)
    ...
    def _generate_pkgbuild(self, meta: PackageMetadata, resolved_deps: list[str] | None = None, license_id: str = "") -> str:
    ...
    def _on_makepkg_finished(self, exit_code: int, _status: QProcess.ExitStatus) -> None:
        ...
        if pkg_file:
            ...
            final = finalize_build_output(
                self._work_dir / "native_build", self._work_dir
            )
            final = final or pkg_file
            self.output_line.emit(tr("nativedeb.native_deb_paketi_hazirlandi", pkg_file_name=final.name))
            self.finished.emit(True, "Native DEB dönüşümü başarılı", final)
        else:
