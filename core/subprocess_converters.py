...
class NativeDebConverterSubprocess:
    ...
    def _do_convert(self, deb_path: Path, output_dir: Path) -> None:
        ...
        try:
            ...
            # Unknown upstream license → honest LicenseRef + stub file so the
            # built package passes namcap instead of a systematic ERROR.
            from core.maps import arch_license, write_license_stub

            lic = arch_license(getattr(meta, "license", "") or "", meta.name)
            if lic.startswith("LicenseRef-"):
                write_license_stub(src_dir, meta.name)

            # Generate PKGBUILD
            pkgbuild = self._generate_pkgbuild(meta, resolved_deps, lic)
    ...
    def _generate_pkgbuild(self, meta, resolved_deps, license_id: str = ""):
        ...
        desc = pkgbuild_literal(
            meta.description or f"{name} (converted from DEB)")
        deps = " ".join(pkgbuild_literal(d) for d in (resolved_deps or []))
        lic = license_id or arch_license(getattr(meta, "license", "") or "", name)
...
pkgdesc={desc}
arch=('{arch}')
url={pkgbuild_literal(meta.url or "https://archlinux.org")}
license=('{lic}')
depends=({deps})
...
"""
    ...
    def _run_makepkg(self, build_dir: Path, output_dir: Path):
        ...
        if pkg_file:
            # Move the artifact to the output root, drop src/ + pkg/ build
            # waste, keep PKGBUILD for review.
            from core.maps import finalize_build_output

            final = finalize_build_output(build_dir, output_dir)
            self.finished.emit(True, "Native DEB dönüşümü başarılı", final or pkg_file)
        else:
...
class RpmConverterSubprocess:
    ...
    def _rpm_makepkg_build(self, meta, output_dir: Path) -> tuple[bool, str]:
        ...
        # Unknown upstream license → honest stub so namcap stays clean.
        from core.maps import arch_license, write_license_stub

        if arch_license(getattr(meta, "license", "") or "", meta.name).startswith("LicenseRef-"):
            write_license_stub(src_dir, meta.name)
    ...
    def _do_convert(self, rpm_path: Path, output_dir: Path, meta=None):
        try:
            ...
            if pkg_file:
                ...
                final = finalize_build_output(output_dir / "build", output_dir)
                self.finished.emit(True, "RPM dönüşümü başarılı", final or pkg_file)
            else:
                self.finished.emit(False, "makepkg başarılı ama çıktı paketi bulunamadı", None)
    ...
    def _generate_pkgbuild(self, meta, src_dir, license_id: str = ""):
        ...
        dep_names: list[str] = []
        if resolved:
            dep_names = list(resolved)
        else:
            for dep in meta.depends:
                mapped = RPM_DEP_MAP.get(dep)
                if mapped:
                    dep_names.append(mapped)
        ...
        desc = pkgbuild_literal(
            meta.description or f"{name} (converted from RPM)")
        deps_str = " ".join(pkgbuild_literal(d) for d in dep_names)
        lic = license_id or arch_license(getattr(meta, "license", "") or "", name)
...
pkgdesc={desc}
arch=('{arch}')
url={pkgbuild_literal(meta.url or "https://unknown")}
license=('{lic}')
depends=({deps_str})
options=('!strip' '!emptydirs')
