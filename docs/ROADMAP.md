...
- **Faz 5 — Mükemmelleştirme** (Faz 4 sonrası eleştiriler C1–C20 + 20 yaratıcı öneri S1–S20 → F5.1–F5.26): ✅ **TAMAMLANDI (26/26)**. Sertleştirme/borç: F5.1 capabilities tek-kaynak + D-Bus arayan-UID (fail-closed -32002), F5.2a token-file/env + rate-map budama, F5.2b --insecure-http-lan + --trusted-proxy XFF, F5.3 systemd sandbox, F5.4 pkexec tek helper (pkgforge-privileged.sh) + eylem-bazlı exec.path, F5.5 swagger vendor + CSP + HTML→http_assets, F5.6 keyring canlı CI + OpenSession marshalling, F5.7 hypothesis import_backup fuzz, F5.9 83 legacy ruff sıfırlandı, F5.10a badge + Cargo parity, F5.11 api_server→core/http_api/ (meta+read router, facade), F5.12 kuyruk sqlite kalıcılığı+restore, F5.13 METHODS→TS codegen + CI diff-check. Ürün: F5.14 install.rehearse (konteyner provası), F5.15 provenance makbuzu (araç sürümleri+debtap-db→attest/SBOM), F5.16 delta DENEYSEL etiketi, F5.17 restore_drill görevi, F5.18 fleet sync backends (git/rclone-s3)+age, F5.19 politika motoru+Settings, F5.20 Convert canlı dil, F5.21 AppStream+flatpak, F5.22 pkgforge doctor, F5.23 SSE GET /events, F5.24 Stats Wrapped, F5.25 upstream enrichment, F5.26 perf bütçesi (%20). Testler: **906 Python (+6 skip), 74 vitest**; ruff/mypy temiz; desktop build temiz.
  ...
  - **F5.8b mutmut security.py genişletmesi (2026-08-27, Tur-65):** mutmut ikinci kritik modüle (`core/security.py`, 613 satır / 651 mutant) genişletildi. Taban çizgisi: **301 öldürüldü / 272 hayatta / 76 testsiz / 2 timeout (~%52)** — bu, "%100 satır kapsaması"nın test kalitesini ölçmediğini somut kanıtladı (installer'daki %34 tabanıyla aynı örüntü). `tests/test_security_mutmut_round65.py` (26 test) eklendi: kesin sınır değerleri (128 paket adı, validate_file_size tam MB sınırları), tam hata/uyarı mesajları, subprocess-mock ile `validate_mime_type`/`safe_run`/`run_sandboxed` kapsaması. Sonuç: **336 öldürüldü / 313 hayatta / 2 timeout / 0 testsiz** (+35; önceki 76 testsiz mutant artık koşuyor). Kalan 313'ün büyük kısmı `build_sandbox_cmd`/`check_compression_bomb`/`run_sandboxed`/imza-doğrulama gibi karmaşık fonksiyonlarda ve önemli bölümü eşdeğer mutant (bwrap flag sırası, mesaj metni, chunk boyutu vb.) — getiri azalması nedeniyle burada duruldu. Config artık `installer.py + security.py`'yi kapsar. ✅ **KAPANIŞ (2026-08-27):** birleşik koşu (installer+security) **880 mutant / 564 öldürüldü / 314 hayatta / 2 timeout**. `check_path_traversal`'daki gerçek sınır mutantı (`len(parts)>1`→`>2`) yeni bir edge-case testiyle öldürüldü (`test_security_pure_logic.py::test_absolute_single_level_nonfhs_flagged`); kalan 314 örneklenerek incelendi ve büyük çoğunluğunun eşdeğer mutant (`f.read(8192)`→`f.read(None)`, `\x7fELF`→`\x7FELF` aynı bayt, `strict=False`→`None`) ya da log-string mutantı olduğu kanıtlandı — ayrıntı `findings.md` MUTMUT-EŞDEĞER bölümünde.
