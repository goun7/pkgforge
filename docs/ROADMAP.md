...
- **Faz 5 — Mükemmelleştirme** (Faz 4 sonrası eleştiriler C1–C20 + 20 yaratıcı öneri S1–S20 → F5.1–F5.26): 🚧 devam ediyor. Tamamlananlar: F5.1 capabilities tek-kaynak + D-Bus arayan-UID doğrulaması (fail-closed -32002), F5.2a token-file/env + rate-map budama, F5.2b loopback-dışı düz HTTP'ye --insecure-http-lan onayı + --trusted-proxy XFF, F5.3 systemd sandbox direktifleri, F5.6 keyring canlı CI (gnome-keyring) + OpenSession marshalling düzeltmesi, F5.7 hypothesis import_backup fuzz (yol-güvenliği invariant), F5.9 83 legacy ruff bulgusu sıfırlandı (CI tam repo), F5.10a badge + Cargo parity, F5.13 METHODS→TS codegen + CI diff-check, F5.20 Convert canlı dil. Testler: 792 Python (+3 skip), 74 vitest.
  - **F5.8 mutmut pilot durumu:** mutmut 3.7 kuruldu ve `pyproject.toml [tool.mutmut]` yapılandırıldı (kaynak: core/installer.py, test: tests/test_converter_guards.py). Ancak mutmut 3.x kaynakları izole bir `mutants/` dizinine kopyalıyor; bu projenin üst-seviye modülleri (`config`, `i18n`) o dizinde import edilemediği için conftest `ModuleNotFoundError` veriyor ve 0 mutant üretiliyor. CI'a alınmadı (plan gereği). Çözüm yolu: ya conftest'in mutants dizininde proje kökünü `sys.path`'e eklemesi, ya da mutmut'un izole-kopya davranışını kapatacak bir runner yapılandırması. Takip eden fazda ele alınacak.
