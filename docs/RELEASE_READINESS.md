...
| Metric | Value | Notes |
|--------|-------|-------|
| Test suite | **567 passed, 12 skipped, 0 failed** | PyQt6 present; green |
| Line coverage (`core/`) | **51%** (6,274 stmts, 3,050 miss) | CI gate: 48% (raised from 35%) |
...
security-clean (bandit 0 High/Medium), and green (567 tests). A broad refactor
...
3. Raise `core/` coverage from 51% toward 60% (Qt-coupled converter paths are
...
3. ~~**Coverage 41%**~~ — now **51%** (567 tests). The happy-path risk called out
